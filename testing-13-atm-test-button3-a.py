...
def mainPage():
    ...
    # ================= TOMBOL SIMULASI TAMBAHAN =================
    # Tombol kecil untuk simulasi penambahan 1 botol (testing)
    # Ditempatkan di pojok kanan bawah

    sim_btn_w = min(120, int(frame_w * 0.10))
    sim_btn_h = min(35, int(frame_h * 0.06))
    sim_btn_x = frame_w - sim_btn_w - 20  # 20px padding dari kanan
    sim_btn_y = frame_h - sim_btn_h - 20  # 20px padding dari bawah
    ...
    makeBtn(
        ...
        "#6c757d",
        "#868e96",
        simulasikanBotol,  # fungsi baru untuk simulasi
        sim_btn_x,
        sim_btn_y,
        sim_btn_w,
        sim_btn_h,
        bg_color="white",
        font_size=10  # font lebih kecil
    )
...
def simulasikanBotol():
    """
    Fungsi untuk simulasi penambahan 1 botol (tanpa sensor fisik).
    Digunakan untuk testing.
    """

    global last_state

    # Set status menjadi True (seperti sensor mendeteksi)
    output = True

    # Update label sensor (simulasi)
    sensor1Label.config(text="1")
    sensor2Label.config(text="1")
    ...
    # Jalankan fungsi counter
    bottleCounter()

    # Set output GPIO LOW (seperti deteksi sensor)
    GPIO.output(OUTPUT_PIN, GPIO.LOW)

    print(
        "[SIMULASI] +1 botol ditambahkan (testing)"
    )

    # Reset status setelah 0.5 detik
    root.after(
        500,
        lambda: resetSimulasiStatus()
    )


def resetSimulasiStatus():
    """
    Reset status setelah simulasi selesai.
    """
    ...
    # Reset label sensor
    sensor1Label.config(text="0")
    sensor2Label.config(text="0")
